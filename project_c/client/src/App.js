...
function App() {
  ...
  useEffect(() => {
    // First check if we have a session in Project C
    checkProjectCSession();
    ...
    if (localStorage.getItem('projectA_logout') === 'true') {
      console.log('Found existing logout flag - logging out from Project C');
      setUser(null);
      localStorage.removeItem('projectA_logout');
    }
    
    // Set up polling to check Project C session status
    const intervalId = setInterval(checkProjectCSessionStatus, 5000);
  ...
  const handleStorageEvent = (event) => {
    if (event.key === 'projectA_logout' && event.newValue === 'true') {
      console.log('Detected logout from Project A - logging out from Project C');
      setUser(null);
      
      // Clear Project C session via API
      fetch('http://localhost:3003/api/auth/logout', {
        method: 'POST',
        credentials: 'include'
      }).catch(error => {
        console.error('Error clearing Project C session:', error);
      });
    }
  };
  
  // Periodically check if Project C session is still valid
  const checkProjectCSessionStatus = async () => {
    ...
    try {
      ...
      if (!data.isLoggedIn && user) {
        console.log('Detected Project A session expired - logging out from Project C');
        setUser(null);
        
        // Clear Project C session
        fetch('http://localhost:3003/api/auth/logout', {
          method: 'POST',
          credentials: 'include'
        }).catch(error => {
  ...
  // Check if we're already logged in to Project C
  const checkProjectCSession = async () => {
    try {
      ...
      if (data.isLoggedIn) {
        // We have a session in Project C
        setUser(data.user);
        setShowLoginModal(false);
        setLoading(false);
      } else {
        // No session in Project C, try silent auth with Project A
        checkSilentAuth();
      }
    } catch (error) {
      console.error('Project C session check error:', error);
      checkSilentAuth();
    }
  };
  ...
  const checkSilentAuth = async () => {
    try {
      const response = await fetch('http://localhost:3003/api/silent-auth', {
        credentials: 'include'
      });
      const data = await response.json();

      if (data.isLoggedIn) {
        // User is logged in to Project A, create a session in Project C
        const verifyResponse = await fetch('http://localhost:3003/api/auth/verify-token', {
          method: 'POST',
          headers: {
    ...
    try {
      ...
      function handleAuthMessage(event) {
        ...
        if (event.data && event.data.type === 'AUTH_SUCCESS') {
          ...
          // Create a session in project C
          createSessionInProjectC(event.data.user);
  ...
  // Function to create a session in Project C
  const createSessionInProjectC = async (user) => {
    try {
      // Call the verification endpoint to create a session
      const response = await fetch('http://localhost:3003/api/auth/verify-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!data.success) {
        console.error('Failed to create session in Project C');
        setUser(null);
      }
    } catch (error) {
    ...
    <div style={{ padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      {user ? (
        <div>
          ...
          <p>You are logged in to Project C via Project A (SSO).</p>
          <button 
      ...
      {showLoginModal && !user && (
        <div style={{
          ...
          <div style={{
            ...
            <h2>Login to Project C</h2>
            <p>To access Project C, you need to login with your Project A account.</p>
