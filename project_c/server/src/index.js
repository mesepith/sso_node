...
app.use(cors({
  origin: process.env.CLIENT_ORIGIN || 'http://localhost:3013',
  credentials: true
}));
...
app.get('/api/auth/callback', async (req, res) => {
  try {
    ...
    const tokenSet = await oidcClient.callback('http://localhost:3013/callback', { code, state }, { nonce });
    ...
    // Set session cookie
    res.cookie('projectC_sessionId', sessionId, { 
      httpOnly: true,
...
// Check auth status endpoint for Project C
app.get('/api/auth/status', (req, res) => {
  // Check for session cookie
  const sessionId = req.cookies.projectC_sessionId;
  
  if (sessionId && sessions.has(sessionId)) {
    // Session exists, user is logged in to Project C
    ...
    // No valid session, user is not logged in to Project C
    res.json({ 
      isLoggedIn: false 
    });
  }
...
app.post('/api/auth/logout', async (req, res) => {
  ...
  if (sessionId) {
    ...
    // Clear cookie
    res.clearCookie('projectC_sessionId');
  }
...
// Endpoint for Project A to trigger logout in Project C
...
app.listen(PORT, () => {
  console.log(`Project C server running on port ${PORT}`);
});